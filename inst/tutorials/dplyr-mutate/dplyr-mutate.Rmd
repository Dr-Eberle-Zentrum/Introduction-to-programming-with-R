...
# 'HH' can be extracted using integer division by 100, i.e. 'dep_time %/% 100'
# 'MM' can be extracted using modulo (rest division), i.e. 'dep_time %% 100'
# since midnight is represented by '2400', it would be converted to 1440 minutes (24*60 + 00)
# -> thus, we have to do a final rest division by 1440 to handle the special case.
# eventually, what we want is: (HH*60 + MM) %% 1440
select(flights, dep_time) %>% 
  mutate( 
