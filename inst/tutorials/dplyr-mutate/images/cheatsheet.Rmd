...
- **creation functions** 
  - are done *rowwise* on the *whole column* at once
  ...
  - common operations: 
    ...
    - offset, i.e. column is shifted up or down by `n` rows
      - `lag()` = previous values, i.e. shifting the column down (`NA`s at the beginning)
      - `lead()` = following values, i.e. shiting the column up (`NA`s at the end)
