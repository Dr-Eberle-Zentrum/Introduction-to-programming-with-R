...
Correct the following code, such that all flights with `month` equal to 1 are selected.
...
```{r hadley-filter-flights-solution}
filter(flights, month == 1)
...
### [3.1 Piping](https://moderndive.com/3-wrangling.html#piping)
...
```{r piping-flights-solution, exercise=TRUE}
flights %>%
  filter(carrier == "AS")
# or in one row
...
### [3.2 `filter` rows](https://moderndive.com/3-wrangling.html#filter)
