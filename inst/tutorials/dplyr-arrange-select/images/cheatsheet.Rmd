...
- `arrange()` changes the order of rows
  ...
  - `NA` is always last
- `select()` reduces the columns to variables of interest
  - comma-separated variable list or vector notation
  ...
  - `-` negation removes the specified variables (also works for vectors via `c()` or `:`)
